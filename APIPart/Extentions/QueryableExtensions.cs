...
namespace APIPart.Extentions
{
    public static class QueryableExtensions
    {
        public async static Task<PagingModel<TInput>> GetPagedResult<TInput>
        (this IQueryable<TInput> query, int currentPage,
        int rowsPerPage,
        string orderBy,
        bool getAllRecords,
        string sortOrder = "asc") where TInput : class
        {
            ...
            var result = getAllRecords ? await finalQuery.ToListAsync() : await
                finalQuery.Skip(itemsToSkip).Take(rowsPerPage).ToListAsync();
